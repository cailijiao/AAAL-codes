...
def create_buffers(obs_shape, num_actions, flags) -> Buffers:
    ...
    specs = dict(
        ...
        #reward_in3=dict(size=(T + 1,), dtype=torch.float32),
...
def act(i: int, free_queue: mp.SimpleQueue, full_queue: mp.SimpleQueue,
    ...
    try:
        ...
        while True:
            ...
            buffers['reset_state_count'][index][0, ...] = torch.tensor(env_output['reward_in3'])

            n_s = _get_count(state_key, state_count_dict)
            #buffers['state_count'][index][0, ...] = torch.tensor(1 / np.sqrt(n_s))
            buffers['state_count'][index][0, ...] = torch.tensor(env_output['reward_in3'])
            ...
            for t in range(flags.unroll_length):
                ...
                buffers['state_count'][index][t + 1, ...] = torch.tensor(env_output['reward_in3'])

                buffers['change_count'][index][t + 1, ...] = torch.tensor(1 / np.sqrt(n_c))
                #buffers['sum_count'][index][t + 1, ...] = torch.tensor(1 / np.sqrt(n_s + n_c))
                #buffers['sum_count'][index][t + 1, ...] = torch.tensor(1/n_c + env_output['reward_in3'])
                buffers['sum_count'][index][t + 1, ...] = torch.tensor(env_output['reward_in3'])   ###########  in_r  dot
                #buffers['sum_count'][index][t + 1, ...] = torch.tensor(0.0)
                #buffers['sum_count'][index][t + 1, ...] = torch.tensor(1 / n_c)
                ...
                buffers['reset_state_count'][index][t + 1, ...] = torch.tensor(env_output['reward_in3'])
                ...
                #buffers['reset_sum_count'][index][t + 1, ...] = torch.tensor(1/n_c + env_output['reward_in3'])
                buffers['reset_sum_count'][index][t + 1, ...] = torch.tensor(env_output['reward_in3'])   ###### in_r   dot
                #buffers['reset_sum_count'][index][t + 1, ...] = torch.tensor(0.0)
                #buffers['reset_sum_count'][index][t + 1, ...] = torch.tensor(1 / n_c)
