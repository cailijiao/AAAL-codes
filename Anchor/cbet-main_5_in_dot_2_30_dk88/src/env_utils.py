...
class EnvironmentMiniGrid:
    ...
    def step(self, action):
        ...
        if len(self.envs_cluster[self.env_id])==0 and len(self.envs_history[self.env_id]) == 1:
            new_c = self.fuc_c(self.envs_history[self.env_id])
            self.envs_cluster[self.env_id].append(new_c)
        else:
