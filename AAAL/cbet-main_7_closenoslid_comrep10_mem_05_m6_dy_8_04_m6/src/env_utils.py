...
class EnvironmentMiniGrid:
    def __init__(self, gym_envs, no_task=False, fixed_seed=None, act_id=-1):
        ...
        self.n_keepstep=30  #30   #10   #15
        self.n_cluster=30   #30   #3
        self.n_keepstep_mem=10   #10   #15
        self.sum_mem=-0.4    #-0.5 # 0.05
        self.sum_disc=0.8
    ...
    def initial(self):
        ...
        for _ in range(len(self.all_envs)):
            ...
            self.envs_history_mem.append(deque(maxlen=self.n_keepstep_mem))
...
#self.fuc_inr_mem(self.n_keepstep_mem, self.envs_history_mem[self.env_id])
    def fuc_inr_mem(self, states):
        #in_a=self.sum_mem/self.n_keepstep_mem
        np_states = np.array(states)
        len_s = len(states)
        in_r_sum = 0

        if len_s > 1:
            all_states = np_states[:-1].reshape(len_s - 1, -1)
            now_states = np_states[-1].reshape(1, -1)
            d_data = (((all_states-now_states) ** 2).sum(axis=1)) ** 0.5

            np_zero=np.where(d_data == 0.0)[0]
            if len(np_zero) > 0:
                n_min = len_s - 1 - (np_zero.max())
                in_r_sum = self.sum_mem * (self.sum_disc ** n_min)

            #num_rep=len(np.where(d_data==0.0)[0])
            #in_r_sum=in_a*num_rep
            #min_dis = d_data[d_data.argmin()]
            #in_r_sum -= 1.0 / (min_dis + yita_2)
    ...
    def step(self, action):
        ...
        self.envs_history_mem[self.env_id].append(pano.numpy())
        self.envs_history[self.env_id].append(pano.numpy())
        np_pano=pano.numpy()
        in_r=self.fuc_inr(np_pano, self.envs_cluster[self.env_id])
        in_r_2 = self.fuc_inr_mem(self.envs_history_mem[self.env_id])
        #print("*************   in_r     in_r_2    in_r_sum   *********")
        #print(in_r)
        #print(in_r_2)
        in_r+=in_r_2
        #print(in_r)
        '''
        if len(self.envs_history[self.env_id]) > 1:
            ll = (((self.envs_history[self.env_id][-1].reshape(1,-1) - self.envs_history[self.env_id][-2].reshape(1,-1)) ** 2).sum(axis=1)) ** 0.5
            in_r_2 = -1.0 / (ll[0] + yita)
            min_line = -1.0 / (0.0 + yita)
            if in_r_2 == min_line:
                in_r-=0.05
                #in_r *=1.5
        '''
        ...
        if done:
            ...
            self.envs_history_mem[self.env_id].clear()
